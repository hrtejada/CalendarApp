...
if(isset($_POST["username"])){
	...
	$sqli="SELECT id,username,level FROM ".T_BOOKINGS_ADMIN." WHERE username='".mysqli_real_escape_string($username)."' AND password='".md5(mysqli_real_escape_string($password))."' AND state=1";
	...
	if(mysqli_num_rows($res)==0){
		...
		mysqli_query($update) or die("error updating user visit stats");
